 ...
 *
 * @export
 * @param {*} backend 一个对象，包含 nodeOps 和 modules
 * @returns {Function} 返回一个 path 函数
 */
export function createPatchFunction (backend) {
    ...
    for (; startIdx <= endIdx; ++startIdx) {
      ...
      if (isDef(ch)) {
        if (isDef(ch.tag)) {
          ...
          // 并对它的子节点递归调用 removeAndInvokeRemoveHook。
          removeAndInvokeRemoveHook(ch)
          // 执行 module 的 destroy 钩子函数以及 vnode 的 destroy
          // 钩子函数
  ...
  /**
   * patch 函数
   * @param {VNode} oldVnode 旧的 VNode 节点
   * @param {VNode} vnode 执行 render 后得到的 VNode 节点
